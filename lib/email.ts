...
export async function sendVerificationEmail({ to, url }: VerificationEmailPayload) {
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #1f2937;">
      ...
      <p>Welcome to ListologyAi! Click the button below to activate your account.</p>
  ...
  await transporter.sendMail({
    to,
    from,
    subject: "Confirm your email for ListologyAi",
    html,
  });
}
...
export async function sendPasswordResetEmail({ to, url }: PasswordResetEmailPayload) {
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #1f2937;">
      <h2 style="color:#4f46e5;">Reset your ListologyAi password</h2>
  ...
  await transporter.sendMail({
    to,
    from,
    subject: "Reset your ListologyAi password",
    html,
  });
}
