...
export default function Pricing() {
  ...
  return (
    <div className="relative py-12 sm:py-16">
      <div className="absolute inset-x-0 top-0 -z-10 h-64 bg-gradient-to-b from-purple-200/40 via-transparent to-transparent blur-3xl" />
      ...
      <section className="relative overflow-hidden rounded-3xl border border-white/80 bg-white/80 px-6 py-16 shadow-soft-xl backdrop-blur sm:px-10 lg:px-16">
        <div className="absolute -left-24 top-20 h-56 w-56 rounded-full bg-purple-400/30 blur-3xl" aria-hidden="true" />
        <div className="absolute -right-20 bottom-10 h-64 w-64 rounded-full bg-fuchsia-400/30 blur-3xl" aria-hidden="true" />
        ...
        <div className="relative mx-auto max-w-4xl text-center">
          <span className="inline-flex items-center justify-center rounded-full bg-gradient-to-r from-purple-600 via-fuchsia-500 to-amber-400 px-4 py-1 text-xs font-semibold uppercase tracking-[0.35em] text-white">
            Pricing
          </span>
          <h1 className="mt-6 text-pretty text-4xl font-semibold text-slate-900 sm:text-5xl">
          ...
          <div className="mt-8 flex flex-wrap items-center justify-center gap-3 text-xs font-semibold uppercase tracking-[0.3em] text-slate-500">
            {valueProps.map((prop) => (
              <span
                key={prop}
                className="rounded-full border border-slate-200 bg-white/80 px-3 py-1 shadow-inner shadow-white/60"
              >
                {prop}
              </span>
            ))}
        ...
        <div className="mt-10 flex justify-center">
          <fieldset aria-label="Payment frequency">
            <RadioGroup
              ...
              className="grid grid-cols-2 gap-1 rounded-full bg-slate-100/80 p-1 text-xs font-semibold text-slate-500"
            >
              {pricing.frequencies.map((option) => (
                <Radio
                  key={option.value}
                  value={option}
                  className="cursor-pointer rounded-full px-4 py-2 transition data-[checked]:bg-slate-900 data-[checked]:text-white data-[checked]:shadow-lg"
                >
                  {option.label}
                </Radio>
              ))}
            ...
            return (
              <div
                ...
                className={classNames(
                  "relative flex h-full flex-col overflow-hidden rounded-3xl border border-slate-200/80 bg-white/80 p-8 shadow-inner shadow-white/60 transition",
                  tier.mostPopular
                    ? "ring-2 ring-purple-400 shadow-lg shadow-purple-200/60"
                    : "hover:-translate-y-1 hover:border-purple-200 hover:shadow-lg"
                )}
              >
                {tier.mostPopular && (
                  <span className="absolute left-1/2 top-0 -translate-y-1/2 -translate-x-1/2 rounded-full bg-gradient-to-r from-purple-600 via-fuchsia-500 to-amber-400 px-4 py-1 text-xs font-semibold text-white shadow-lg shadow-fuchsia-400/40">
                    Most loved
                  </span>
                )}
                <div className="absolute inset-x-8 top-0 h-px bg-gradient-to-r from-purple-400/40 via-fuchsia-400/30 to-amber-400/40" aria-hidden="true" />
                <div className="flex flex-col gap-4">
                  ...
                  <button
                    ...
                    className={classNames(
                      ...
                      tier.mostPopular
                        ? "bg-gradient-to-r from-purple-600 via-fuchsia-500 to-amber-400 text-white shadow-lg shadow-fuchsia-400/40 hover:opacity-95"
                        : "border border-slate-200 bg-white/80 text-purple-600 hover:border-purple-200 hover:text-purple-700",
                  ...
                  <ul role="list" className="mt-6 space-y-3 text-sm leading-6 text-slate-600">
                    {tier.features.map((feature) => (
                      <li key={feature} className="flex items-center gap-3">
                        <CheckIcon aria-hidden="true" className="h-5 w-5 text-purple-500" />
                        <span>{feature}</span>
                      </li>
                    ))}
                  </ul>
      ...
      <section className="relative mt-20 rounded-3xl border border-white/80 bg-white/80 px-6 py-12 shadow-soft-xl backdrop-blur sm:px-10">
      ...
      <section className="relative mt-24 overflow-hidden rounded-3xl border border-white/80 bg-white/80 px-6 py-16 shadow-soft-xl backdrop-blur sm:px-10 lg:px-16">
        <div className="absolute -left-16 top-10 h-48 w-48 rounded-full bg-purple-400/25 blur-3xl" aria-hidden="true" />
        <div className="absolute -right-24 bottom-10 h-60 w-60 rounded-full bg-fuchsia-400/25 blur-3xl" aria-hidden="true" />
        <div className="relative mx-auto max-w-3xl">
          ...
          <dl className="mt-10 space-y-4">
            {faqs.map((faq) => (
              <Disclosure key={faq.question} as="div" className="rounded-3xl border border-slate-200/80 bg-white/90 p-5 shadow-inner shadow-white/60">
                {({ open }) => (
                  <>
                    <dt>
                      <DisclosureButton className="flex w-full items-center justify-between text-left">
                        <span className="text-sm font-semibold text-slate-900">{faq.question}</span>
                        <span className="ml-4 flex h-7 w-7 items-center justify-center rounded-full border border-slate-200 bg-white text-slate-500">
